# source( "~/labo/src/FeatureEngineering/z815_FE_final_Caro.r")
# Se corrió en Google Cloud
# 256 GB de memoria RAM (luego noté que podría haber utilizado mucho menos, 64GB por ejemplo)
...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , invento_Caro01 := ctransferencias_recibidas  / ccuenta_debitos_automaticos ]
  dataset[ , invento_Caro02 := catm_trx  / catm_trx_other ]
  dataset[ , invento_Caro03 := mv_mlimitecompra  / (mtarjeta_visa_debitos_automaticos + mtarjeta_master_debitos_automaticos) ]
  dataset[ , invento_Caro04 := mdescubierto_preacordado  / mcuentas_saldo ]
  dataset[ , invento_Caro05 := mdescubierto_preacordado  / mautoservicio ]
  dataset[ , invento_Caro06 := mdescubierto_preacordado  / mtarjeta_master_consumo ]
  dataset[ , invento_Caro07 := mdescubierto_preacordado  / mtarjeta_visa_consumo ]
  dataset[ , invento_Caro08 := mrentabilidad  * cliente_antiguedad ]
  dataset[ , invento_Caro09 := mcuenta_corriente  / mcuenta_corriente_adicional ]
  dataset[ , invento_Caro10 := mean(mautoservicio) ]
  dataset[ , invento_Caro11 := Visa_mlimitecompra  / Visa_mconsumototal ]
  dataset[ , invento_Caro12 := Master_mlimitecompra  / master_mconsumototal ]
  dataset[ , invento_Caro13 := rowSums( cbind(mpayrroll, mpayroll2), na.rm=TRUE)]
  dataset[ , invento_Caro14 := invento_Caro13 / cpayroll2_trx ]
  dataset[ , invento_Caro15 := pmax( Master_delinquency,  Visa_delinquency, na.rm = TRUE) ]
  dataset[ , invento_Caro16 := mpayrroll / cpayroll_trx ]
  dataset[ , invento_Caro17 := matm_other / catm_trx]
  dataset[ , invento_Caro18 := mforex_buy  / mpayroll ]
  dataset[ , invento_Caro19 := mcuenta_debitos_automaticos  / cpagodeservicios ]
  dataset[ , invento_Caro20 := mplazo_fijo_pesos  / mpayroll ]
...
#cargo el dataset
dataset  <- fread( "./datasets/competencia1_historia_2022_IPCCaro.csv.gz" )
...
#grabo el dataset
fwrite( dataset,
        "dataset_7130_FEfinal_Caro.csv.gz",
        logical01= TRUE,
        sep= "," )
